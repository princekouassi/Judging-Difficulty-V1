...
bootstrap_cis = function(data){
  ...
  for (i in 1:n){
    data_mean = mean(sample(real_data,replace=T)) # Get the mean for one interation
    mean_list[[i]] = data_mean # Store the mean of the ith iteration
  }
